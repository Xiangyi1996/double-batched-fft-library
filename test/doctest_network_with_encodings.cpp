...
void test_network_with_encoding_training_loaded(sycl::queue &q, std::string filepath, const int n_hidden_layers,
    ...
    //  std::vector<double> stacked_dL_doutput_ref = stack_vector(dL_doutput_ref, batch_size);
    //  dL_doutput.copy_from_host(convert_vector<double, T>(stacked_dL_doutput_ref)).wait();

    //  Net->backward_pass(const DeviceMatrix<T_net> &input_backward,
...
void test_network_with_encoding_identity_backward(sycl::queue &q, const int input_width, const int output_width,
    ...
    DeviceMatrix<T> dL_dinput(batch_size, Net->get_network()->get_input_width(), q);
    auto dL_input_view = dL_dinput.GetView();
...
test_create_nwe_as_shared_ptr(sycl::queue &q, const int encoding_input_width, const json &encoding_config) {
...
//                                                             weight_init_mode);
//     else if (width == 128)
//         test_network_with_encoding_identity_backward<T, 128>(q, 128, 128, n_hidden_layers, batch_size,
//         activation,
//                                                              weight_init_mode);
//     else
//         throw std::invalid_argument("Unsupported width");
...
TEST_CASE("tinydpcppnn::network_with_encoding step-by-step") {
    ...
    //                                {EncodingParams::OFFSET, 0.0},
    //                                {EncodingParams::ENCODING, EncodingNames::IDENTITY}};
    //     test_create_nwe_as_shared_ptr<float, bf16, 64>(q, encoding_input_width, encoding_config);
    // }
    ...
    //     net = test_create_nwe_as_shared_ptr<float, bf16, 64>(q, encoding_input_width, encoding_config);

    //     DeviceMem<float> params_full_precision(net->get_encoding()->n_params(), q);

    //     params_full_precision.fill(1.0f).wait();

    //     DeviceMem<float> gradients(net->get_encoding()->n_params(), q);
    //     gradients.fill(0.123f).wait(); // fill with something to check if it is written to

    //     net->set_encoding_params(params_full_precision, gradients);
    // }
