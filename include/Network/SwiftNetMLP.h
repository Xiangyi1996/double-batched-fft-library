...
void dgemm_multiply(queue q, bf16 *grads_device, float *loss_gradients, float *fwd, float *A, float *B, float *C, int k,
                    int m_n_hidden_matrices, int batch_size, int m_inputs_width, int &flops) {
    ...
    if (k == (n_hidden_matrices - 1)) {
        ...
        offset_g = (m_inputs_width + (n_hidden_matrices - k - 2) * WIDTH) * batch_size;
        offset_c = (m_inputs_width * WIDTH + (n_hidden_matrices - k - 2) * WIDTH * WIDTH);
    }
    ...
    q.parallel_for<>(range<1>(layer_in_width * batch_size), [=](id<1> idx) {
         ...
         A[i * batch_size + j] = (float)elt_activation_ret<float>(ACTIVATION, fwd[i + j * layer_in_width + offset_g]);
         // int b_first;
         // int b_second;
         // int b_zeroes;
    ...
    oneapi::mkl::blas::row_major::gemm(q, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans,
                                       layer_in_width, WIDTH, batch_size, 1, A, batch_size, B, WIDTH, 0, C, WIDTH);
    ...
    // Calculating flops
    // A Matrix
    flops = (layer_in_width * batch_size) + 2 * layer_in_width * WIDTH * batch_size;
...
template <int WIDTH> class SwiftNetMLP : public Network {
  public:
    ...
    int get_padded_output_width() const override { return m_padded_output_width; }
