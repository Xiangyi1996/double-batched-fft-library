...
class DeviceMem {
 ...
 public:
  ...
  void copy_from_host(const T *host_data, const size_t num_elements) {
    dpct::get_default_queue().memcpy(data(), host_data,
  ...
  void copy_to_host(T *host_data, const size_t num_elements) const {
    if (num_elements > m_size) {
      throw std::runtime_error{
          fmt::format("Trying to copy {} elements, but memory size is only {}.",
                      num_elements, m_size)};
    }

    dpct::get_default_queue().memcpy(host_data, data(),
                                     num_elements * sizeof(T));
  }
  ...
  void copy_to_host(std::vector<T> &data, const size_t num_elements) const {
    if (data.size() < num_elements) {
      throw std::runtime_error{
          fmt::format("Trying to copy {} elements, but vector size is only {}.",
  ...
  /// it
  void copy_from_device(const DeviceMem<T> &other) {
  ...
  T &at(size_t idx) const {
    if (!m_managed) {
      throw std::runtime_error{
          fmt::format("DeviceMem::at() not permitted if not managed.")};
    }

    if (idx > m_size) {
      throw std::runtime_error{
          fmt::format("DeviceMem out of bounds: idx={} size={}", idx, m_size)};
    }
 ...
 public:
  DeviceMemArena() {
    ...
    try {
      ze_result_t res;
      m_device =
          sycl::get_native<backend::ext_oneapi_level_zero>(q.get_device());
      m_context =
          sycl::get_native<backend::ext_oneapi_level_zero>(q.get_context());

      size_t total_memory{
          q.get_device().get_info<sycl::info::device::global_mem_size>()};

      size_t page_size;
      res = zeVirtualMemQueryPageSize(m_context, m_device, total_memory,
                                      &page_size);
      if (res != ZE_RESULT_SUCCESS)
        throw std::runtime_error{"DeviceMemArena: Could not query page-size."};
      ...
      m_alignment =
          q.get_device()
              .get_info<sycl::info::device::global_mem_cache_line_size>();
      ...
      res =
          zeVirtualMemReserve(m_context, nullptr, m_max_size, &m_base_address);
      if (res != ZE_RESULT_SUCCESS) {
        m_base_address = nullptr;
        throw std::runtime_error{
            "DeviceMemArena: Could not reserve address space."};
      }
      ...
      if (!printed_warning) {
        ...
        log_warning(ex.what());
        log_warning(
            "DeviceMemArena: GPU {} does not support virtual memory. "
  ...
  ~DeviceMemArena() {
    ...
    try {
      ...
      if (m_base_address) {
        ...
        if (res != ZE_RESULT_SUCCESS)
          throw std::runtime_error{"~DeviceMemArena: Could not unmap memory."};

        while (!m_handles.empty()) {
          res = zePhysicalMemDestroy(m_context, m_handles.back());
          if (res != ZE_RESULT_SUCCESS)
            throw std::runtime_error{
                "~DeviceMemArena: Could not destroy physical memory "
                "allocation."};
          m_handles.pop_back();
        }
        ...
        if (res != ZE_RESULT_SUCCESS)
          throw std::runtime_error{
              "~DeviceMemArena: Could not destroy address reservation."};
      }
  ...
  uint8_t *data() {
    return m_fallback_memory ? m_fallback_memory->data()
                             : reinterpret_cast<uint8_t *>(m_base_address);
  }
  ...
  void free(size_t start) {
    if (m_allocated_intervals.count(start) == 0)
      throw std::runtime_error{
          "Attempted to free arena memory that was not allocated."};
    ...
    m_free_intervals.insert(
        std::upper_bound(std::begin(m_free_intervals),
                         std::end(m_free_intervals), interval),
        interval);
  ...
  void enlarge(size_t n_bytes) {
    ...
    if (dpct::get_current_device_id() != m_device_id)
      throw std::runtime_error{
          fmt::format("Attempted to use a DeviceMemArena of device {} from the "
                      "wrong device {}.",
                      m_device_id, dpct::get_current_device_id())};
    ...
    if (m_fallback_memory) {
      ...
      if (res != ZE_RESULT_SUCCESS)
        throw std::runtime_error{
            "DeviceMemArena::enlarge: Could not query page-size."};
    ...
    size_t page_size;
    res = zeVirtualMemQueryPageSize(m_context, m_device, n_bytes_to_allocate,
                                    &page_size);
    if (res != ZE_RESULT_SUCCESS)
      throw std::runtime_error{
          "DeviceMemArena::enlarge: Could not query page-size."};
    ...
    res =
        zePhysicalMemCreate(m_context, m_device, &pmemDesc, &m_handles.back());
    if (res != ZE_RESULT_SUCCESS)
      throw std::runtime_error{
          "DeviceMemArena::enlarge: Could not allocate physical memory."};

    // Map into virtual address space
    res = zeVirtualMemMap(m_context,
                          reinterpret_cast<uint8_t *>(m_base_address) + m_size,
                          n_bytes_to_allocate, m_handles.back(), 0,
                          ZE_MEMORY_ACCESS_ATTRIBUTE_READWRITE);
    if (res != ZE_RESULT_SUCCESS)
      throw std::runtime_error{
          fmt::format("DeviceMemArena::enlarge: Could not map memory into "
