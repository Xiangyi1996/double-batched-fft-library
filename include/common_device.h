...
inline float logit(const float x) {
  return -sycl::log(
      1.0f / (sycl::fmin(sycl::fmax((float)x, 1e-9f), 1.0f - 1e-9f)) - 1.0f);
}

template <uint32_t N>
inline void softmax(float vals[N]) {
  float total = 0;
...
template <uint32_t N>
inline float softmax(const float vals[N], uint32_t idx) {
  float total = 0;
...
template <typename T>
T relu(T val) {
  return (T)sycl::max((float)val, 0.0f);
}

template <>
inline sycl::half relu(sycl::half val) {
...
void warp_activation(Activation activation, const fragment_t& frag,
                     fragment_t& result) {
  switch (activation) {
    ...
    case Activation::Softplus:
      ...
      for (int t = 0; t < result.num_elements; t++) {
        result.x[t] =
            (T)(sycl::log(sycl::exp((float)frag.x[t] * K_ACT) + 1.0f) / K_ACT);
      }
      return;
...
template <typename T, typename fragment_t>
fragment_t warp_activation(Activation activation, const fragment_t& frag) {
  fragment_t result;
...
template <typename T, typename fragment_t, typename forward_fragment_t>
fragment_t warp_activation_backward_in(
...
template <typename T, typename fragment_t, typename forward_fragment_t>
void warp_activation_backward(Activation activation, const fragment_t& frag,
  ...
  switch (activation) {
    ...
    case Activation::LeakyReLU:
      ...
      for (int t = 0; t < result.num_elements; t++) {
        result.x[t] =
            frag.x[t] * (T)(forward_frag.x[t] > (T)0.0f ? 1.0f : 0.01f);
      }
      return;
    case Activation::Exponential:
    ...
    case Activation::Softplus:
      ...
      for (int t = 0; t < result.num_elements; t++) {
        result.x[t] = frag.x[t] *
                      (T)(1.0f - sycl::exp(-(float)forward_frag.x[t] * K_ACT));
      }
      return;
    case Activation::Tanh:
...
template <typename T, typename fragment_t, typename forward_fragment_t>
fragment_t warp_activation_backward(Activation activation,
...
void sh_enc(uint32_t degree, float x, float y, float z, ARRAY_T& data_out,
  ...
  data_out[1 + offset] =
      (T)(-0.48860251190291987f * y);  // -sqrt(3)*y/(2*sqrt(pi))
  data_out[2 + offset] =
      (T)(0.48860251190291987f * z);  // sqrt(3)*z/(2*sqrt(pi))
  data_out[3 + offset] =
      (T)(-0.48860251190291987f * x);  // -sqrt(3)*x/(2*sqrt(pi))
  if (degree <= 2) {
    return;
  }
  data_out[4 + offset] =
      (T)(1.0925484305920792f * xy);  // sqrt(15)*xy/(2*sqrt(pi))
  data_out[5 + offset] =
      (T)(-1.0925484305920792f * yz);  // -sqrt(15)*yz/(2*sqrt(pi))
  data_out[6 + offset] =
      (T)(0.94617469575755997f * z2 -
  ...
  data_out[12 + offset] =
      (T)(0.3731763325901154f * z *
          (5.0f * z2 - 3.0f));  // sqrt(7)*z*(5*z2 - 3)/(4*sqrt(pi))
  data_out[13 + offset] =
      (T)(0.45704579946446572f * x *
  ...
  data_out[49 + offset] = (T)(0.70716273252459627f * y *
                              (-21.0f * x2 * y4 + 35.0f * x4 * y2 - 7.0f * x6 +
                               y6));  // 3*sqrt(715)*y*(-21*x2*y4 + 35*x4*y2 -
                                      // 7*x6 + y6)/(64*sqrt(pi))
  data_out[50 + offset] = (T)(5.2919213236038001f * xy * z *
                              (-10.0f * x2 * y2 + 3.0f * x4 +
                               3.0f * y4));  // 3*sqrt(10010)*xy*z*(-10*x2*y2 +
  ...
  data_out[58 + offset] = (T)(0.07375544874083044f * z * (x2 - y2) *
                              (143.0f * z2 * (3.0f * z2 - 1.0f) - 187.0f * z2 +
                               45.0f));  // sqrt(70)*z*(x2 - y2)*(143*z2*(3*z2 -
  ...
  data_out[60 + offset] =
      (T)(1.0378311574405206f * z * (13.0f * z2 - 3.0f) *
          (-6.0f * x2 * y2 + x4 + y4));  // 3*sqrt(385)*z*(13*z2 - 3)*(-6*x2*y2
                                         // + x4 + y4)/(32*sqrt(pi))
  data_out[61 + offset] =
      (T)(-0.51891557872026028f * x * (13.0f * z2 - 1.0f) *
          (-10.0f * x2 * y2 + x4 +
           5.0f * y4));  // -3*sqrt(385)*x*(13*z2 - 1)*(-10*x2*y2 +
                         // x4 + 5*y4)/(64*sqrt(pi))
  data_out[62 + offset] = (T)(2.6459606618019f * z *
                              (15.0f * x2 * y4 - 15.0f * x4 * y2 + x6 -
                               y6));  // 3*sqrt(10010)*z*(15*x2*y4 - 15*x4*y2 +
                                      // x6 - y6)/(64*sqrt(pi))
  data_out[63 + offset] =
...
void sh_enc(uint32_t degree, float x, float y, float z, ARRAY_T& data_out) {
  ...
  data_out(31) = (T)(0.45294665119569694f * x *
                     (14.0f * z2 - 21.0f * z4 -
                      1.0f));  // sqrt(165)*x*(14*z2 - 21*z4 - 1)/(16*sqrt(pi))
  data_out(32) = (T)(2.3967683924866621f * z * (x2 - y2) *
                     (3.0f * z2 -
                      1.0f));  // sqrt(1155)*z*(x2 - y2)*(3*z2 - 1)/(8*sqrt(pi))
  data_out(33) =
      (T)(-0.48923829943525038f * x * (x2 - 3.0f * y2) *
...
template <typename T, typename ARRAY_T>
tnn::vec3 sh_enc_grad(uint32_t degree, float x, float y, float z,
                      const ARRAY_T& dL_dy) {
  // Let compiler figure out how to sequence/reorder these calculations w.r.t.
  // branches
  float xy = x * y, xz = x * z, yz = y * z, x2 = x * x, y2 = y * y, z2 = z * z;
  ...
  d.z +=
      (float)dL_dy(6) * (1.8923493915151202 * z);  // 3*sqrt(5)*z/(2*sqrt(pi))
  d.x +=
  ...
  d.y += (float)dL_dy(10) *
         (2.8906114426405538 * xz);  // sqrt(105)*xz/(2*sqrt(pi))
  d.z += (float)dL_dy(10) *
         (2.8906114426405538 * xy);  // sqrt(105)*xy/(2*sqrt(pi))
  ...
  d.z += (float)dL_dy(44) *
         (5.5272315570895412 * z * (x2 - y2) *
          (11.0 * z2 -
           3.0));  // 3*sqrt(2730)*z*(x2 - y2)*(11*z2 - 3)/(16*sqrt(pi))
  d.x += (float)dL_dy(45) *
         (-2.7636157785447706 * z * (x2 - y2) *
          (11.0 * z2 -
           3.0));  // -3*sqrt(2730)*z*(x2 - y2)*(11*z2 - 3)/(32*sqrt(pi))
  ...
  d.y += (float)dL_dy(46) *
         (-2.0182596029148963 * y * (3.0 * x2 - y2) *
          (11.0 * z2 -
           1.0));  // -3*sqrt(91)*y*(3*x2 - y2)*(11*z2 - 1)/(8*sqrt(pi))
  d.z += (float)dL_dy(46) *
         (11.10042781603193 * z *
          (-6.0 * x2 * y2 + x4 +
  ...
  d.y += (float)dL_dy(53) * (-0.46937680158688211 * (x2 - y2) *
                             (13.0 * z2 * (11.0 * z2 - 3.0) - 27.0 * z2 +
                              3.0));  // -9*sqrt(35)*(x2 - y2)*(13*z2*(11*z2 -
  ...
  d.z +=
      (float)dL_dy(56) * (64.528641681844675 * z2 - 236.60501950009714 * z4 +
  ...
  d.x += (float)dL_dy(61) *
         (-0.51891557872026028 * (13.0 * z2 - 1.0) *
          (-10.0 * x2 * y2 + 4.0 * x2 * (x2 - 5.0 * y2) + x4 +
           5.0 * y4));  // -3*sqrt(385)*(13*z2 - 1)*(-10*x2*y2 + 4*x2*(x2 -
...
uint32_t lcg_hash(const tnn::uvec<N_DIMS>& pos_grid,
...
uint32_t prime_hash(const tnn::uvec<N_DIMS>& pos_grid) {
...
template <uint32_t N_DIMS>
uint32_t coherent_prime_hash(const tnn::uvec<N_DIMS>& pos_grid) {
...
template <uint32_t N_DIMS>
uint32_t reversed_prime_hash(const tnn::uvec<N_DIMS>& pos_grid) {
...
uint32_t grid_hash(const tnn::uvec<N_DIMS>& pos_grid) {
  switch (HASH_TYPE) {
    case HashType::Prime:
...
template <uint32_t N_DIMS, HashType HASH_TYPE>
uint32_t grid_index(const GridType grid_type, const uint32_t hashmap_size,
...
inline float grid_scale(uint32_t level, float log2_per_level_scale,
...
inline uint32_t grid_resolution(float scale) {
  return (uint32_t)sycl::ceil(scale) + 1;
}

template <typename T, uint32_t N, size_t A = sizeof(T)>
using vector_fragment_t = VectorFragment<tnn::tvec<T, N, A>>;

template <typename T, uint32_t N = 1>
void kernel_activation(const uint32_t num_elements, const Activation act,
                       const T* in, T* out, const sycl::nd_item<3>& item_ct1) {
...
template <typename T, uint32_t N = 1>
void kernel_activation_backward(const uint32_t num_elements,
...
template <typename T, uint32_t N = 1>
void kernel_activation_backward_output(const uint32_t num_elements,
                                       const Activation act,
...
inline void pos_fract(const float input, float* pos, float* pos_derivative,
                      float* pos_2nd_derivative, uint32_t* pos_grid,
...
template <typename F>
inline void pos_fract(const float input, float* pos, uint32_t* pos_grid,
                      float scale, F interpolation_fun) {
  // The offset of 0.5 causes different scales to be staggered with respect to
  // each other, thus preventing spurious alignment of fractional coordinates
  // upon integer scales (or powers thereof). This is mentioned in Appendix A of
...
inline float weight_decay(float relative_weight_decay,
                          float absolute_weight_decay, float weight) {
  ...
  return (1 - relative_weight_decay) * weight -
         sycl::copysign(absolute_weight_decay, weight);
}
...
inline float gaussian_cdf_approx_derivative(const float result,
...
inline float gaussian_pdf(const float x, const float inv_radius) {
  return inv_radius * sycl::rsqrt(2.0f * PI) *
         sycl::exp(-0.5f * (x * x * inv_radius * inv_radius));
}
...
inline float tent(const float x, const float inv_radius) {
  return sycl::fmax(1.0f - sycl::fabs(x * inv_radius), 0.0f);
}
...
inline float quartic_cdf_deriv(const float x, const float inv_radius) {
...
inline float quartic_cdf(const float x, const float inv_radius) {
  ...
  return sycl::fmax(0.0f, sycl::fmin(1.0f, ((float)15 / 16) * u *
                                                   (1 - ((float)2 / 3) * u2 +
                                                    ((float)1 / 5) * u4) +
                                               0.5f));
}
...
template <typename T>
void shuffle(const uint32_t n_elements, const uint32_t stride,
             const uint32_t seed, const T* __restrict__ in, T* __restrict__ out,
             const sycl::nd_item<3>& item_ct1) {
  const uint32_t i = item_ct1.get_local_id(2) +
                     item_ct1.get_group(2) * item_ct1.get_local_range(2);
...
template <typename T>
void fill_rollover(const uint32_t n_elements, const uint32_t stride,
...
template <typename T>
void fill_rollover_and_rescale(const uint32_t n_elements, const uint32_t stride,
                               const uint32_t* n_input_elements_ptr, T* inout,
                               const sycl::nd_item<3>& item_ct1) {
  const uint32_t i = item_ct1.get_local_id(2) +
                     item_ct1.get_group(2) * item_ct1.get_local_range(2);
...
void add(const uint32_t num_elements, const T1* data_in_1, const T2* data_in_2,
         T3* data_out, const sycl::nd_item<3>& item_ct1) {
  const uint32_t i = item_ct1.get_local_id(2) +
                     item_ct1.get_group(2) * item_ct1.get_local_range(2);
  if (i >= num_elements) return;
...
template <typename T>
void add(const uint32_t num_elements, const T* __restrict__ data_in,
         T* __restrict__ data_in_out, const sycl::nd_item<3>& item_ct1) {
  const uint32_t i = item_ct1.get_local_id(2) +
                     item_ct1.get_group(2) * item_ct1.get_local_range(2);
  if (i >= num_elements) return;
...
template <typename T>
void trim(const uint32_t num_elements, const uint32_t stride,
...
template <typename T>
void trim_and_cast(const uint32_t num_elements, const uint32_t stride,
                   const uint32_t dims, const T* __restrict__ data_in,
...
template <typename T>
void cast(const uint32_t num_elements, const float* __restrict__ full_precision,
          T* __restrict__ target, const sycl::nd_item<3>& item_ct1) {
  const uint32_t i = item_ct1.get_local_id(2) +
                     item_ct1.get_group(2) * item_ct1.get_local_range(2);
  if (i >= num_elements) return;
...
template <typename T>
void cast_from(const uint32_t num_elements, const T* __restrict__ precision,
...
template <typename T>
void extract_dimension_pos_neg_kernel(const uint32_t num_elements,
                                      const uint32_t dim, const uint32_t fan_in,
                                      const uint32_t fan_out,
                                      const T* __restrict__ encoded,
...
template <typename T>
void mult_scalar_kernel(const uint32_t num_elements, T* __restrict__ inout,
                        float factor, const sycl::nd_item<3>& item_ct1) {
  const uint32_t i = item_ct1.get_local_id(2) +
                     item_ct1.get_group(2) * item_ct1.get_local_range(2);
  if (i >= num_elements) return;
...
template <typename T>
void mult_kernel(const uint32_t num_elements, const T* factor1,
