 ...
 public:
  ...
  std::unique_ptr<ForwardContext> training_step(
    ...
    return training_step(nullptr, input, target, data_pdf, run_optimizer,
                         dL_dinput, use_inference_params, param_gradients_mode,
                         external_dL_dy);
  }
  ...
  void training_step(DeviceMem<bf16>& input, DeviceMem<float>& output,
    ...
    m_optim->step(
        ...
        m_encoding->m_weightsT_matrices, m_encoding->m_grads_matrices, WIDTH);
  }
  ...
  std::unique_ptr<ForwardContext> forward(
    ...
    // GPUMatrix<COMPUTE_T>{m_model->padded_output_width(), batch_size, stream};
    ...
    // auto& loss_input = m_perturbation_sigma > 0 ? forward->perturbed_output :
    // forward->output;

    // forward->L = GPUMatrix<float>{m_model->padded_output_width(), batch_size,
    // stream};
    ...
    // 	forward->dL_doutput = GPUMatrix<COMPUTE_T>{external_dL_dy->data(),
    ...
    // 	forward->dL_doutput =
    // GPUMatrix<COMPUTE_T>{m_model->padded_output_width(), batch_size, stream};
    // 	m_loss->evaluate(stream, loss_scale, loss_input, target, forward->L,
  ...
  void backward(dpct::queue_ptr stream, const ForwardContext& ctx,
    ...
    m_encoding->backward_impl(stream, *ctx.model_ctx, input, ctx.output,
                              ctx.dL_doutput, dL_dinput, use_inference_params,
                              param_gradients_mode);
  }
