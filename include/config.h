...
TrainableModel create_from_config(queue q, json config) {
  ...
  std::string loss_type =
      config.value("loss", json::object()).value("otype", "RelativeL2");
  std::string optimizer_type =
      config.value("optimizer", json::object()).value("otype", "sgd");
  const int WIDTH =
      config.value("network", json::object()).value("n_neurons", 64);
  ...
  if (isequalstring(optimizer_type, "Adam")) {
    ...
    optimizer = new SGDOptimizer(
        config.value("optimizer", json::object()).value("output_width", 64),
  ...
  switch (WIDTH) {
    case 64:
      network = new SwiftNetMLP<64>(
          ...
          string_to_activation(config.value("network", json::object())
                                   .value("activation", "ReLU")),
          string_to_activation(config.value("network", json::object())
                                   .value("output_activation", "None")),
    ...
    default:
      throw std::runtime_error{
          "SwiftNetMLP only supports 64, and 128 neurons, but got ..."};
  }
