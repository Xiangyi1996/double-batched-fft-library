...
class SwiftNetMLPFactory {
  public:
    static std::unique_ptr<Network> create(queue q, int width, int input_width, int output_width, int n_hidden_layers,
                                           Activation activation, Activation output_activation) {
        switch (width) {
        case 64:
            return std::make_unique<SwiftNetMLP<64>>(q, input_width, output_width, n_hidden_layers, activation,
                                                     output_activation);
        default:
            throw std::runtime_error("SwiftNetMLP not implemented for the specified width: " + std::to_string(width));
        }
    }
};
...
class Module {
  ...
  protected:
    ...
    template <typename T> std::vector<T> get_vector_from_tensor(torch::Tensor tensor) {
    ...
    template <typename T> void convert_tensor_to_dev_mem(torch::Tensor &tensor, DeviceMem<T> *device_mem_array) {
    ...
    template <typename T> torch::Tensor get_converted_tensor_from_array(T *array, int size) {
...
class EncodingModule : public Module {
  public:
    EncodingModule(int input_width, std::string encoding_name,
