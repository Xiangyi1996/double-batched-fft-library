 ...
 */
void sgd_step(id<1> idx, const int n_elements, int output_width,
              int n_hidden_layers, const float loss_scale,
              const float learning_rate, const float l2_reg, bf16* weights,
              bf16* gradients, int WIDTH) {
  ...
  if (matrices_number < n_hidden_layers) {
    ...
    packed_idx_matrices =
        toPackedLayoutCoord(matrices_offset, WIDTH, output_width);
  }
 ...
 * @param WIDTH           The width of weight matrices.
 */
void sgd_stepT(id<1> idx, const int n_elements, int output_width,
               int n_hidden_layers, const float loss_scale,
               const float learning_rate, const float l2_reg, bf16* weightsT,
               bf16* gradients, int WIDTH) {
  ...
  if (matrices_number < n_hidden_layers) {
    ...
    packed_idx_matrices =
        fromPackedLayoutCoord(matrices_offset, output_width, WIDTH);
  }
...
SGDOptimizer::SGDOptimizer(int output_rows, int n_hidden_layers,
                           float learning_rate, float l2_reg) {
...
void SGDOptimizer::step(queue q, float loss_scale, DeviceMem<bf16>& weights,
  ...
  q.parallel_for<>(range<1>(n_elements), [=](id<1> idx) {
     sgd_step(idx, n_elements, output_rows, n_hidden_layers, loss_scale,
              learning_rate, l2_reg, weights.data(), gradients.data(), WIDTH);
   }).wait();

  // Perform the SGD update for transposed weight matrices
  q.parallel_for<>(range<1>(n_elements), [=](id<1> idx) {
     sgd_stepT(idx, n_elements, output_rows, n_hidden_layers, loss_scale,
               learning_rate, l2_reg, weightsT.data(), gradients.data(), WIDTH);
   }).wait();
}
