 ...
 */
void Relative_L2_loss(id<1> idx, const int n_elements, const int dims, const int stride, const float scale,
 ...
 */
void RelativeL2Loss::evaluate(queue q, const int dims, const int stride, const float scale, DeviceMem<float> &preds,
    ...
    q.parallel_for<>(range<1>(n_elements), [=](id<1> idx) {
         ...
         Relative_L2_loss(idx, n_elements, dims, stride, scale, preds.data(), targets.data(), grads.data(),
                          values.data());
     }).wait();
}
