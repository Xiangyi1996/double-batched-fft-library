...
EncodingModule::EncodingModule(int input_width, std::string encoding_name,
...
torch::Tensor EncodingModule::backward_pass(torch::Tensor input_tensor, torch::Tensor grad_output,
                                            torch::Tensor params) {}

void EncodingModule::free_memory() {}

NetworkWithEncodingModule::NetworkWithEncodingModule(
    int width, int input_width, int output_width, int n_hidden_layers, Activation activation,
    Activation output_activation, std::string encoding_name,
    const std::unordered_map<std::string, std::string> &encoding_config, std::string device_name)
    ...
    network = new NetworkWithEncoding(input_width, output_width, n_hidden_layers, activation, output_activation,
                                      encoding_name, encoding_config);
}

torch::Tensor NetworkWithEncodingModule::forward_pass(torch::Tensor input_tensor, torch::Tensor params,
                                                      int use_inference) {
    set_params(params);
    int batch_size = input_tensor.sizes()[1];
...
torch::Tensor NetworkWithEncodingModule::backward_pass(torch::Tensor input_tensor, torch::Tensor grad_output,
                                                       torch::Tensor params) {
    int batch_size = input_tensor.sizes()[1];
...
void NetworkWithEncodingModule::initialize_params(float *params_full_precision, int use_easy) {
...
int NetworkWithEncodingModule::n_params() { return network->get_network()->get_weights_matrices()->size(); }
