...
torch::Tensor NetworkWithEncodingModule::forward_pass(
  ...
  GPUMatrix<float> input_matrix = GPUMatrix<float>(
      input_tensor.data_ptr<float>(), m_input_width, batch_size);
  DeviceMem<float> output = network->forward_pass(input_matrix, use_inference);
...
torch::Tensor NetworkWithEncodingModule::backward_pass(
  ...
  //       DeviceMem<bf16>(batch_size * input_width, sycl_queue);
  //   DeviceMem<bf16> grads =
  //       DeviceMem<bf16>(batch_size * output_width, sycl_queue);
  ...
  //       network->backward_pass(input_backward, grads, batch_size);

  //   torch::Tensor grad_loss =
  //   get_converted_tensor_from_dev_mem(grads_matrices);
