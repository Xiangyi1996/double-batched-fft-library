...
//   DeviceMem<float> input_f = DeviceMem<float>(batch_size * input_width, q);
//   DeviceMem<float> output_f = DeviceMem<float>(batch_size * output_width, q);
//   DeviceMem<float> target_f = DeviceMem<float>(batch_size * output_width, q);
...
//     2.0f, \
//     false, \
//     string_to_interpolation_type("Linear"), \
//     string_to_grid_type("Hash"), \

//   Encoding<float>* encoding = new GridEncodingTemplated<float,
//   input_width/*n_dims_to_encode*/, 2/*N_FEATURES_PER_LEVEL*/,
//   HashType::CoherentPrime>{ TCNN_GRID_PARAMS };
...
//   input_f.initialize_constant(0.01f, q);
//   output_f.initialize_constant(0.0f, q);
//   target_f.initialize_constant(1.0f, q);

//   GPUMatrix<float> input_float(input_f.data(), batch_size * input_width,
//                                batch_size * output_width);
//   GPUMatrix<float> output_float(output_f.data(), batch_size * input_width,
//                                 batch_size * output_width);
//   GPUMatrix<float> target_float(target_f.data(), batch_size * input_width,
//                                 batch_size * output_width);
...
//   std::cout << "Output dim: " << output_float.rows() << " x "
//             << output_float.cols() << std::endl;

//   std::vector<float> output(output_float.cols() * output_float.rows());
//   q.memcpy(output.data(), output_float.data(), output.size() *
//   sizeof(float)); q.wait();

//   std::vector<float> input(input_float.cols() * input_float.rows());
//   q.memcpy(input.data(), input_float.data(), input.size() * sizeof(float));
//   q.wait();

//   std::cout << "Forward pass" << std::endl;
//   std::cout << "Stride : " << output_float.stride() << std::endl;

//   // Iterate through the elements and access their values
//   for (uint32_t i = 0; i < output_float.rows(); ++i) {
//     for (uint32_t j = 0; j < output_float.cols(); ++j) {
//       float value = output[j + i];
//       std::cout << "row: " << i << ", col: " << j << ": " << value <<
//       std::endl; std::cout << "row: " << i << ", col: " << j << ": " <<
//       input[j + i]
//                 << std::endl;
//     }
//   }
...
//   DeviceMem<float> dli_f = DeviceMem<float>(batch_size * input_width, q);
//   DeviceMem<float> dlo_f = DeviceMem<float>(batch_size * output_width, q);
...
//   std::vector<float> output_train(output_float.cols() * output_float.rows());
//   if(output_train.empty() || !ctx->output.data()) {
//       std::cerr << "Error: No training data! Empty..." << std::endl;
//       exit(0);
//   }

//   q.memcpy(output_train.data(), ctx->output.data(),
//            output_train.size() * sizeof(float));
//   q.wait();

//   std::vector<float> target(target_float.c`ols() * target_float.rows());
//   q.memcpy(target.data(), target_float.data(), target.size() *
//   sizeof(float)); q.wait();

//   for (uint32_t i = 0; i < output_float.rows(); ++i) {
//     for (uint32_t j = 0; j < output_float.cols(); ++j) {
//       std::cout << "Input: row: " << i << ", col: " << j << ": " << input[j +
//       i]
//                 << std::endl;
//       std::cout << "Output: row: " << i << ", col: " << j << ": "
//                 << output[j + i] << std::endl;
//       std::cout << "Target: row: " << i << ", col: " << j << ": "
//                 << target[j + i] << std::endl;
//     }
//   }
