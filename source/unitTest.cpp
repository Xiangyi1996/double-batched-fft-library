...
void test_exactitude() {
  ...
  float* forward =
      malloc_device<float>(batch_size * (WIDTH + output_width + WIDTH * 2), q);
  ...
  std::vector<MultilayerPerceptron::TrainingElement> training_set(
      1,
      MultilayerPerceptron::TrainingElement(x, std::vector<float>(64, 1.0f)));
  ...
  for (int j = 0; j < 64; j++) {
    for (int k = 0; k < 64; k++) {
      res[j + 64] +=
          res[k] * w_swift[64 * 64 + toPackedLayoutCoord(k * 64 + j, 64, 64)];
    }
  }
  for (int j = 0; j < 64; j++) {
    for (int k = 0; k < 64; k++) {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    std::cout << my_mlp.layers[3].err[i] * my_mlp.layers[2].out[0] << " ; "
              ...
              << std::endl;
    std::cout << my_mlp.layers[3].err[i] * my_mlp.layers[2].out[0] << " ; "
              << grad[2 * 64 * 64 + 2 * 64 + i] << " ; " << l_grad[2 * 64 + i]
              << std::endl;
    std::cout << my_mlp.layers[3].err[i] * my_mlp.layers[2].out[0] << " ; "
              << grad[2 * 64 * 64 + 3 * 64 + i] << " ; " << l_grad[3 * 64 + i]
              << std::endl;
  }
...
void test_encoding() {
  ...
  //   DeviceMem<float> output_f = DeviceMem<float>(batch_size * output_width,
