...
void NetworkWithEncoding::forward_pass(GPUMatrix<float> &input, int run_inference, DeviceMem<float> &network_output,
    ...
    float *A_forward = sycl::aligned_alloc_device<float>(m_alignment, input_width * net_width, m_q);
    float *B_forward = sycl::aligned_alloc_device<float>(m_alignment, output_width * net_width, m_q);
    float *C_forward = sycl::aligned_alloc_device<float>(m_alignment, output_width * batch_size, m_q);
    ...
    if (run_inference) {
        network->inference(network_input, forward, A_forward, B_forward, C_forward, network_output, batch_size);
    } else {
        network->forward_pass(network_input, forward, A_forward, B_forward, C_forward, network_output, batch_size);
    }
...
DeviceMem<bf16> *NetworkWithEncoding::backward_pass(DeviceMem<bf16> &input_backward, DeviceMem<bf16> &grad_output,
    ...
    float *A_backward = sycl::aligned_alloc_device<float>(m_alignment, net_width * batch_size, m_q);
    float *B_backward = sycl::aligned_alloc_device<float>(m_alignment, batch_size * output_width, m_q);
    float *C_backward = sycl::aligned_alloc_device<float>(m_alignment, net_width * output_width, m_q);

    float *A_backward_last_layer = sycl::aligned_alloc_device<float>(m_alignment, batch_size * output_width, m_q);
    float *B_backward_last_layer = sycl::aligned_alloc_device<float>(m_alignment, output_width * net_width, m_q);
    float *C_backward_last_layer = sycl::aligned_alloc_device<float>(m_alignment, net_width * batch_size, m_q);
    float *D_backward_last_layer = sycl::aligned_alloc_device<float>(m_alignment, net_width * batch_size, m_q);
    float *E_backward_last_layer = sycl::aligned_alloc_device<float>(m_alignment, batch_size * net_width, m_q);

    float *F_backward_last_layer;
    if (n_hidden_matrices == 0) {
        // in this case, the penultimate layer is the input layer
        F_backward_last_layer = sycl::aligned_alloc_device<float>(m_alignment, input_width * net_width, m_q);
    } else {
        F_backward_last_layer = sycl::aligned_alloc_device<float>(m_alignment, net_width * net_width, m_q);
    }

    float *A_dgemm = sycl::aligned_alloc_device<float>(m_alignment, batch_size * net_width, m_q);
    float *B_dgemm = sycl::aligned_alloc_device<float>(m_alignment, batch_size * net_width, m_q);
    // net_width * net_width is the maximum, for the first layer, it's
    // technically input_width * net_width
    float *C_dgemm = sycl::aligned_alloc_device<float>(m_alignment, net_width * net_width, m_q);

    network->backward_pass(input_backward, grad_output, out_inter, deltas, A_backward, B_backward, C_backward,
                           A_backward_last_layer, B_backward_last_layer, C_backward_last_layer, D_backward_last_layer,
                           E_backward_last_layer, F_backward_last_layer, A_dgemm, B_dgemm, C_dgemm, forward,
                           batch_size);
