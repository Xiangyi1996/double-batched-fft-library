 ...
 * @param queue              SYCL queue associated with the object.
 */
template <typename T> void DeviceMem<T>::allocate2(int size, queue &q) {
    if (m_size != 0 || size <= 0) {
        return;
    }
    m_size = size;
 ...
 * @param queue              SYCL queue associated with the object.
 */
template <typename T> void DeviceMem<T>::allocate(int n_bytes, queue &q) {
    ...
    uint8_t *rawptr{m_managed ? sycl::malloc_shared<uint8_t>(n_bytes + DEBUG_GUARD_SIZE * 2, q)
                              : sycl::malloc_device<uint8_t>(n_bytes + DEBUG_GUARD_SIZE * 2, q)};
    if (!rawptr) throw std::runtime_error{"Allocation failed in line __LINE__ in __FILE__."};
 ...
 * @param queue              SYCL queue associated with the object.
 */
template <typename T> void DeviceMem<T>::allocate(int n_bytes) {
    if (m_size != 0 || n_bytes <= 0) {
        return;
    }
    // m_size = size;
 ...
 * @param queue              SYCL queue associated with the object.
 */
template <typename T> void DeviceMem<T>::free_mem(queue &q) {
    if (!m_data) return;
 ...
 *
 */
template <typename T> void DeviceMem<T>::free_mem() {
    if (!m_data) return;
 ...
 * @param queue              SYCL queue associated with the object.
 */
template <typename T> void DeviceMem<T>::copy_from_host(std::vector<T> &data, int n, queue q) {
 ...
 * @param queue              SYCL queue associated with the object.
 */
template <typename T> void DeviceMem<T>::copy_to_host(std::vector<T> &data, int n, queue q) {
 ...
 * @param queue              SYCL queue associated with the object.
 */
template <typename T> void DeviceMem<T>::copy_from_host(std::vector<T> &data, queue q) {
 ...
 * @param queue              SYCL queue associated with the object.
 */
template <typename T> void DeviceMem<T>::copy_to_host(std::vector<T> &data, queue q) { copy_to_host(data, m_size, q); }
 ...
 * @param value              Value to set.
 */
template <typename T> void DeviceMem<T>::set_data(int id, T value) { m_data[id] = value; }
 ...
 */
template <typename T>
void DeviceMem<T>::initialize_normal(double dev, DeviceMem<T> &transposed, int input_width, int width, int output_width,
    ...
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < output_width; j++) {
            ...
            data[input_width * width + n_hidden * width * width +
                 toPackedLayoutCoord(i * output_width + j, width, output_width)] = rnd;
            dataT[input_width * width + n_hidden * width * width +
                  toPackedLayoutCoord(j * output_width + i, output_width, width)] = rnd;
        }
    }
 ...
 * @param q     The SYCL queue for parallel computation.
 */
template <typename T> void DeviceMem<T>::initialize_normal(double dev, queue q) {
    std::default_random_engine gen;
    std::normal_distribution<double> distrib(0.0, dev);
    std::vector<T> data(m_size);
 ...
 */
template <typename T>
void DeviceMem<T>::initialize_uniform(double scale, DeviceMem<T> &transposed, int input_width, int width,
 ...
 */
template <typename T>
void DeviceMem<T>::make_transposed(DeviceMem<T> &transposed, int input_width, int width, int output_width, int n_hidden,
                                   queue q) {
    auto p = m_data;
    ...
    q.parallel_for<>(range<1>(input_width * width + n_hidden * width * width + width * output_width), [=](id<1> idx) {
         ...
         if (idx < input_width * width) {
             i = idx / input_width;
             j = idx % input_width;
             transposed.data()[toPackedLayoutCoord(j * width + i, input_width, width)] =
             ...
             transposed.data()[input_width * width + mat_num * width * width +
                               toPackedLayoutCoord(j * width + i, width, width)] =
                 p[input_width * width + mat_num * width * width + toPackedLayoutCoord(i * width + j, width, width)];
         } else {
 ...
 */
template <typename T> void DeviceMem<T>::initialize_uniform(queue q, double scale) {
    std::default_random_engine gen;
    std::uniform_real_distribution<double> distrib(0.0, scale);
    std::vector<T> data(m_size);
 ...
 * @param q             The SYCL queue for memory operations.
 */
template <typename T> void DeviceMem<T>::initialize_constant(T constant, DeviceMem<T> &transposed, queue q) {
    std::vector<T> data(m_size, constant);
    q.memcpy(m_data, data.data(), m_size * sizeof(T)).wait();
    q.memcpy(transposed.data(), data.data(), m_size * constant * sizeof(T)).wait();
}

template <typename T> void DeviceMem<T>::initialize_arange(queue q) {
    std::vector<T> data(m_size);
...
template <typename T>
void DeviceMem<T>::initialize_arange(queue q, int input_width, int net_width, int out_width, int hidden_matrices) {
    std::vector<T> data(m_size);
    ...
    for (int k = 0; k < hidden_matrices; ++k) {
        ...
        for (int i = 0; i < net_width; ++i) {
            for (int j = 0; j < net_width; ++j) {
                ...
                data[net_width * input_width + k * net_width * net_width + j * net_width + i] =
                    col_vector_mid[j] * 0.01;
            }
        }
    }
    ...
    for (int i = 0; i < net_width; i++) {
        for (int j = 0; j < out_width; j++) {
            ...
            data[net_width * input_width + hidden_matrices * net_width * net_width + i * out_width + j] =
                col_vector_out[j] * 0.01;
        }
    }
 ...
 * @param q             The SYCL queue for memory operations.
 */
template <typename T> void DeviceMem<T>::initialize_constant(T constant, queue q) {
    auto p = m_data;
    q.parallel_for<>(range<1>(m_size), [=](id<1> idx) { p[idx] = (T)constant; }).wait();
}
...
template <typename T> void DeviceMem<T>::enlarge(const size_t size) {
...
template <typename T> void DeviceMem<T>::memset(const int value) { memset(value, m_size); }
/** @} */
