 ...
 */
void L1_loss(id<1> idx, const int n_elements, const int dims, const int stride,
 ...
 */
void L1Loss::evaluate(queue q, const int dims, const int stride,
  ...
  q.parallel_for<>(range<1>(n_elements), [=](id<1> idx) {
     ...
     L1_loss(idx, n_elements, dims, stride, scale, preds.data(), targets.data(),
