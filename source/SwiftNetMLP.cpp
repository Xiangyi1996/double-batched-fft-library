  ...
  for (int i = 0; i < N_ITERS; i++) {
    for (int k = 0; k < TM; k++) {
      ...
      output_threadblock[TN * sgId * WIDTH + TM * i + k * WIDTH + id] =
          a[TN * sgId + (WIDTH + SKEW) * TM * i + k * WIDTH + id];
    }
  }
}
...
void kernel_swift_mlp(nd_item<1> item, const Activation output_activation,
                      ...
                      const uint32_t output_stride, const uint32_t input_width,
  ...
  if (input_width == WIDTH) {
    ...
    workgroup_matmul_act_dynamic<WIDTH, N_ITERS>(
        item, activation, a, at, input + elem_idx * input_width, weights_layer,
        !INFERENCE ? (out_intermediate_layer + elem_idx * WIDTH) : nullptr,
    ...
    if (INFERENCE) {
      workgroup_write_output_static<WIDTH, N_ITERS>(
          ...
          out_intermediate_layer + elem_idx * WIDTH +
              (n_hidden_matmuls + 1) * layer_lenght);
    }
  } else if (out) {
...
void mlp_swift_forward(queue q, Activation output_activation,
                       ...
                       const int output_stride, const int n_hidden_layers,
     ...
     cgh.parallel_for(
         ...
         [=](nd_item<1> item) [[intel::reqd_sub_group_size(SG_SIZE)]] {
           kernel_swift_mlp<WIDTH, N_ITERS, activation, INFERENCE>(
               ...
               output_stride, input_width, output_width, n_hidden_layers - 1,
               batch_size);
         });
   }).wait();
}
...
void SwiftNetMLP<WIDTH>::forward_pass(const DeviceMem<bf16>& input,
  ...
  m_q.parallel_for<>(range<1>(input.size()), [=](id<1> idx) {
    ...
    // int b_first;
    // int b_second;
    // static const CONSTANT char FMT[] = "input[%d]  %d.%d,";
    // get_float_as_integers_own(input.data()[idx], b_first, b_second);
  ...
  switch (m_activation) {
    case Activation::None:
      mlp_swift_forward<WIDTH, Activation::None, false>(
          ...
          forward + input.size(), output, output_stride, m_n_hidden_layers,
          m_inputs_width, m_output_width, m_batch_size);
      break;
  ...
  //   m_q.parallel_for<>(
  //          range<1>((n_hidden_matrices * WIDTH + m_inputs_width + WIDTH) *
  //                   m_batch_size),
  //          [=](id<1> idx) {
  //            if (idx >= (32 * 64)) {
  //              int b_first;
  ...
  //              get_float_as_integers_own(forward[idx], b_first, b_second);
  //              sycl::ext::oneapi::experimental::printf(FMT, int(idx),
  //              b_first,
  //                                                      b_second);
  //            }
  //          })
...
// template <int WIDTH>
// void SwiftNetMLP<WIDTH>::inference(const DeviceMem<bf16>& input, float*
// forward,
...
//           m_q, m_output_activation, m_weights_matrices, input, forward,
//           output, output_stride, m_n_hidden_layers, m_inputs_width,
...
//         oneapi::mkl::transpose::nontrans, m_batch_size, m_output_width,
//         WIDTH, 1, A, WIDTH, B, m_output_width, 0, output.data(),
//         m_output_width);
//   }
// }
...
void SwiftNetMLP<WIDTH>::backward_pass(
  ...
  m_q.parallel_for<>(range<1>(WIDTH * batch_size),
                     [=](id<1> idx) {
                       ...
                       //    WIDTH + i], b_first,
                       //                              b_second);
                       //    sycl::ext::oneapi::experimental::printf(FMT, int(i
                       //    * batch_size + j),
                       //                                            i, j,
                       //                                            b_first,
                       //                                            b_second);
  ...
  m_q.parallel_for<>(range<1>(m_net_width * m_output_width),
                     [=](id<1> idx) {
                       ...
                       //    int b_first;
                       //    int b_second;
                       //    static const CONSTANT char FMT[] = "p[%d]  %d.%d,";
                       //    get_float_as_integers_own(C[idx], b_first,
                       //    b_second); sycl::ext::oneapi::experimental::printf(
