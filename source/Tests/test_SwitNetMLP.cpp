...
void test1() {
  ...
  */
    ...
    float* forward = malloc_device<float>(batch_size * (WIDTH + output_width + WIDTH * n_hidden_matrices), q);
    ...
    SwiftNetMLP<64> network = SwiftNetMLP<64>(q, 64, 64, n_hidden_layers, Activation::None, Activation::None, batch_size);
    Trainer train(network, loss, optim);

    train.initialize_params();

    q.parallel_for<>(range<1>(inputs.size()), [=](id<1> idx) {
        forward[idx] = inputs.data()[idx];
        });
