...
void work_group_layer(nd_item<1> it, Activation activation, bf16* act_shmem, bf16* weights_layer, T* out_intermediate) {
	...
	device_ptr<float> o(out_intermediate);
	...
	for (int i = 0; i < N_ITERS; i++) {
		for (int j = 0; j < TN; j++) {
			for (int k = 0; k < TM; k++) {
				act_shmem[TN * sgId + TM * i * WIDTH + j + k * WIDTH] = out_intermediate[TN * sgId + TM * i * WIDTH + j + k * WIDTH];
			}
		}
	}
}
...
template <int WIDTH, int N_ITERS>
void workgroup_write_output_static(nd_item<1> item, bf16* act_shmem, float* output_workgoup) {
	...
	for (int i = 0; i < N_ITERS; i++) {
		for (int j = 0; j < TN; j++) {
			for (int k = 0; k < TM; k++) {
				output_workgoup[TN * sgId + TM * i * WIDTH + j + k * WIDTH] = act_shmem[TN * sgId + TM * i * WIDTH + j + k * WIDTH];
			}
		}
	}
}
...
void kernel_swift_mlp(nd_item<1> item,
	...
	for (int k = 0; k < n_hidden_matmuls; k++) {
		work_group_layer<WIDTH, N_ITERS, T, false>(item, activation, act_shmem + elem_idx * WIDTH, weights_layer + first_weight_length + k * hidden_weight_lenght, out_intermediate_layer + elem_idx * WIDTH);
	}

	workgroup_write_output_static(item, act_shmem, output)
...
void test3() {
	...
	std::vector<bf16> weights(WIDTH * WIDTH * (n_hidden_layers + 1), 0);
	std::vector<bf16> packed_weight((n_hidden_layers + 1) * WIDTH * WIDTH, 0);
	std::vector<float> intermediate_output((n_hidden_layers + 1) * batch_size * WIDTH, 0);
	...
	for (int i = 0; i < WIDTH; i++) {
		for (int j = 0; j < WIDTH; j++) {
			weights[WIDTH * WIDTH + WIDTH * i + j] = bf16(1.0f);
			weights[WIDTH * i + j] = bf16(1.0f);
		}
	}

	for (int i = 0; i < WIDTH / 2; i++) {
		for (int j = 0; j < WIDTH; j++) {
			packed_weight[i * WIDTH * 2 + 2 * j] = weights[2 * i * WIDTH + j];
			packed_weight[i * WIDTH * 2 + 2 * j + 1] = weights[(2 * i + 1) * WIDTH + j];
			packed_weight[WIDTH * WIDTH + i * WIDTH * 2 + 2 * j] = weights[WIDTH * WIDTH + 2 * i * WIDTH + j];
			packed_weight[WIDTH * WIDTH + i * WIDTH * 2 + 2 * j + 1] = weights[WIDTH * WIDTH + (2 * i + 1) * WIDTH + j];
		}
	}
	...
	for (int i = 0; i < batch_size; i++) {
		for (int j = 0; j < WIDTH; j++) {
			for (int k = 0; k < WIDTH; k++) {
				res1[i * WIDTH + j] += (float)input[i * WIDTH + k] * (float)weights[k * WIDTH + j];
			}
		}
	}
	for (int i = 0; i < batch_size; i++) {
		for (int j = 0; j < WIDTH; j++) {
			for (int k = 0; k < WIDTH; k++) {
				res2[i * WIDTH + j] += (float)res1[i * WIDTH + k] * (float)weights[WIDTH * WIDTH + k * WIDTH + j];
			}
		}
	}
	for (int i = 0; i < 10; i++) {
