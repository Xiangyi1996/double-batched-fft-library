...
if __name__ == "__main__":
    ...
    for exp in range(20, 22):
        ...
        X_train = torch.Tensor(X_train.astype(np.float16))
        y_train = np.random.rand(batch_size, 64)
        y_train = torch.Tensor(y_train.astype(np.float16))
        ...
        y_pred = network(X_train[0 * batch_size : (0 + 1) * batch_size, ...].flatten())
        ...
        for n in range(1000):
            ...
            for idx in range(X_train.shape[0] // batch_size):
                ...
                loss = loss_fn(
                    y_pred, y_train[idx * batch_size : (idx + 1) * batch_size].flatten()
                )
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
