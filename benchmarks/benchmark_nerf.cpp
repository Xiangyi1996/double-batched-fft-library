...
void start_training(const int WIDTH = 64, const int input_width = 32, const int output_width = 4,
...
#pragma omp parallel num_threads(SubDevices.size()) shared(C, SubDevices)
    {
        ...
        for (uint32_t batch_size : batch_sizes) {
            ...
            std::cout << "Training Time for " << n_iterations << " iterations: " << microseconds_total
...
#pragma omp barrier
            ...
            for (int grads_mat_iter = 0; grads_mat_iter < m_n_hidden_layers + 1; grads_mat_iter++) {
                std::cout << "grads_mat checksum " << grads_mat_iter << " = "
                          << grads_checksum_per_matrix[grads_mat_iter] << std::endl;
            }
...
#pragma omp barrier
            ...
            std::cout << "Inference Time for " << n_iterations << " iterations: " << microseconds_total
                      << "microseconds." << std::endl;
...
#pragma omp barrier
        ...
        if (world_rank == 0) {
            ...
            std::ofstream out{std::string("../benchmarks/results/benchmark_nerf.json")};
            out << json_string;
        }
    }
